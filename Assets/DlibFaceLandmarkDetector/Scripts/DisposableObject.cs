...
namespace DlibFaceLandmarkDetector
{
    abstract public class DisposableObject : IDisposable
    {
        ...
        {
            if (ptr == IntPtr.Zero)
                throw new DlibException("An error occurred on the C++ side, causing the class initialization to fail.Enclose the point where the error occurs in Utils.setDebugMode() method, and the error on the C++ side will be displayed on the console.");
